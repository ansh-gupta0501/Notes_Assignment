...
- this is becasue in the integation request, the mapping template we are sending is not valid json , it is vtl language , so for valid json use **$utils.escapeJavascript($input.json('$'))** with the body 
